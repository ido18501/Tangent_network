...
from training.losses import OperatorVectorLoss
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train and evaluate the tangent operator with full vector-regression losses.")
    ...
    parser.add_argument("--lambda-contrast-first", type=float, default=0.2)
    parser.add_argument("--lambda-contrast-second", type=float, default=0.2)
    parser.add_argument("--contrast-margin-first", type=float, default=0.05)
    parser.add_argument("--contrast-margin-second", type=float, default=0.25)
    parser.add_argument("--lambda-reg", type=float, default=1e-5)
    parser.add_argument("--lambda-zero-sum", type=float, default=1e-2)
    parser.add_argument("--second-min-target-norm", type=float, default=1e-2)
    parser.add_argument("--vector-loss-type", type=str, default="mse", choices=["mse", "smoothl1"])
    parser.add_argument("--smooth-l1-beta", type=float, default=1.0)

    parser.add_argument("--num-viz-examples", type=int, default=9)

    parser.add_argument("--use-precomputed-curves", action="store_true")
    parser.add_argument("--train-curve-dir", type=str, default=None)
    parser.add_argument("--val-curve-dir", type=str, default=None)
    parser.add_argument("--test-curve-dir", type=str, default=None)
...
def make_dataset(args: argparse.Namespace, length: int, seed: int, split: str) -> TangentDataset:
    if args.use_precomputed_curves:
        if split == "train":
            curve_dir = args.train_curve_dir
        elif split == "val":
            curve_dir = args.val_curve_dir
        elif split == "test":
            curve_dir = args.test_curve_dir
        else:
    ...
    return TangentDataset(
        ...
        seed=seed,
        use_precomputed_curves=args.use_precomputed_curves,
        precomputed_curve_dir=curve_dir,
    )
...
def move_batch_to_device(batch: TangentBatch, device: torch.device) -> TangentBatch:
    return TangentBatch(
        ...
        family=batch.family,
        anchor_center_index=batch.anchor_center_index.to(device),
        negative_center_indices=batch.negative_center_indices.to(device),
        gt_first_anchor=batch.gt_first_anchor.to(device),
        gt_second_anchor=batch.gt_second_anchor.to(device),
    )


def forward_batch_and_loss(
    model: nn.Module,
    loss_fn: nn.Module,
    batch: TangentBatch,
    ...
    v_second_neg = neg_out["vector_second"].view(B, M, 2)
    ...
    loss, stats = loss_fn(
        ...
        v_second_negatives=v_second_neg,
        return_stats=True,
    )
    outputs = {
        "anchor_first": anchor_out["vector_first"],
        "anchor_second": anchor_out["vector_second"],
        "positive_first": positive_out["vector_first"],
        "positive_second": positive_out["vector_second"],
    }
...
def run_epoch(
    ...
    for batch in loader:
        ...
        if train:
            optimizer.zero_grad()
            loss, stats, _ = forward_batch_and_loss(model, loss_fn, batch)
            loss.backward()
            ...
            with torch.no_grad():
                _, stats, _ = forward_batch_and_loss(model, loss_fn, batch)
...
def collect_test_predictions(
    ...
    for batch in loader:
        ...
        second_mask = second_norm > second_min_target_norm
...
def format_summary_text(summary: dict[str, Any]) -> str:
    ...
    lines = []
    lines.append("Full-vector tangent-operator test summary (training uses vector regression; cosines shown for evaluation)")
    lines.append("=" * 44)
...
def plot_history(history: list[dict[str, float]], output_path: Path) -> None:
    ...
    train_cos1 = [h.get("train_first_cos_eval", float("nan")) for h in history]
    val_cos1 = [h.get("val_first_cos_eval", float("nan")) for h in history]
    train_cos2 = [h.get("train_second_cos_eval", float("nan")) for h in history]
    val_cos2 = [h.get("val_second_cos_eval", float("nan")) for h in history]
...
def main() -> None:
    ...
    train_dataset = make_dataset(args, args.train_length, args.seed, "train")
    val_dataset = make_dataset(args, args.val_length, args.seed + 10_000, "val")
    test_dataset = make_dataset(args, args.test_length, args.seed + 20_000, "test")
    ...
    loss_fn = OperatorVectorLoss(
        ...
        lambda_contrast_first=args.lambda_contrast_first,
        lambda_contrast_second=args.lambda_contrast_second,
        contrast_margin_first=args.contrast_margin_first,
        contrast_margin_second=args.contrast_margin_second,
        lambda_reg=args.lambda_reg,
        lambda_zero_sum=args.lambda_zero_sum,
        second_min_target_norm=args.second_min_target_norm,
        vector_loss_type=args.vector_loss_type,
    ...
    test_records = collect_test_predictions(model, test_loader, device, args.second_min_target_norm)
