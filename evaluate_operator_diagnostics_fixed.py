...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--test-curve-dir", type=str, default="data/precomputed_curves_fourier_1000/test")
    p.add_argument("--test-length", type=int, default=500)
...
def reproduce_sample(dataset: TangentDataset, index: int) -> dict[str, Any]:
    ...
    anchor_patch_sample = sample_patch_around_index(
        curve_points=curve,
    ...
    # sample_patch_around_index returns CurvePatchSample, not ndarray.
    anchor_patch_global = np.asarray(anchor_patch_sample.points, dtype=np.float32)
    anchor_patch_centered = np.asarray(anchor_patch_sample.centered_points, dtype=np.float32)
    positive_patch_global = apply_transformation(anchor_patch_global, transform).astype(np.float32)
    positive_center = positive_patch_global[dataset.patch_size // 2].copy()
    positive_patch_centered = (positive_patch_global - positive_center[None, :]).astype(np.float32)
    ...
    anchor_center = np.asarray(anchor_patch_sample.center_point, dtype=np.float32)
    curve_centered = (curve - anchor_center[None, :]).astype(np.float32)
    curve_transformed = apply_transformation(curve, transform).astype(np.float32)
    curve_transformed_centered = (curve_transformed - positive_center[None, :]).astype(np.float32)

    anchor_patch = anchor_patch_centered if dataset.return_centered else anchor_patch_global
    positive_patch = positive_patch_centered if dataset.return_centered else positive_patch_global

    return {
        "curve_centered": curve_centered,
        "curve_transformed_centered": curve_transformed_centered,
...
def main() -> None:
    ...
    visualize_examples(rows, output_dir / "qualitative_examples.png", args.num_visualizations, args.high_curvature_threshold)
